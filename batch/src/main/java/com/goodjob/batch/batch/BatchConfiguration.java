...
public class BatchConfiguration {
    ...
    public TaskExecutor taskExecutor() {
        ...
        taskExecutor.setConcurrencyLimit(8); // 기본 스레드 풀 크기
        return taskExecutor;
    }

    // 병렬처리 고민
    @Bean(name = "job")
    public Job job1(JobRepository jobRepository, Step chunkSaram, Step wontedBackStep, Step wontedFrontStep, Step wontedFullStackStep) {
        return new JobBuilder("job1", jobRepository)
//                .incrementer(new RunIdIncrementer())
                .start(saramFlow(chunkSaram))
                .split(taskExecutor())
                .add(wontedBackFlow(wontedBackStep),
                        wontedFrontFlow(wontedFrontStep),
                        wontedFullStackFlow(wontedFullStackStep))
                .end()
//                .start(chunkSaram(jobRepository)) // 청크
//                .next(wontedBackStep(jobRepository))
//                .next(wontedFrontStep(jobRepository))
//                .next(wontedFullStackStep(jobRepository))
                .build();
    }


    @Bean
    public Flow saramFlow(Step chunkSaram) {
        FlowBuilder<Flow> builder = new FlowBuilder<>("사람인");
        builder.start(chunkSaram)
                .end();
        return builder.build();
    }

    @Bean
    public Flow wontedBackFlow(Step wontedBackStep) {
        FlowBuilder<Flow> builder = new FlowBuilder<>("wontedBack");
        builder.start(wontedBackStep)
                .end();
        return builder.build();
    }

    @Bean
    public Flow wontedFrontFlow(Step wontedFrontStep) {
        FlowBuilder<Flow> builder = new FlowBuilder<>("wontedFront");
        builder.start(wontedFrontStep)
                .end();
        return builder.build();
    }
    ...
    public Flow wontedFullStackFlow(Step wontedFullStackStep) {
        FlowBuilder<Flow> builder = new FlowBuilder<>("wontedFullstack");
        builder.start(wontedFullStackStep)
                .end();
        return builder.build();
    }
...
//    @Scope(value = "job", proxyMode = ScopedProxyMode.TARGET_CLASS)
    ...
    public ItemReader<JobResponseDto> saraminReader() {
        ...
        ListItemReader<JobResponseDto> dtos = new ListItemReader<>(jobResponseDtos);
        return dtos;
    }
...
//    @Bean
//    public Job job2(JobRepository repository) {
//        return new JobBuilder("job2", repository)
////                .incrementer(new RunIdIncrementer())
//                .start(wontedBackStep(repository))
//                .build();
//    }
...
//    @Scope(value = "job", proxyMode = ScopedProxyMode.TARGET_CLASS)
    ...
    // Front Wonted
//    @Bean
//    public Job job3(JobRepository repository) {
//        return new JobBuilder("job3", repository)
////                .incrementer(new RunIdIncrementer())
//                .start(wontedFrontStep(repository))
//                .build();
//    }
...
//    @Scope(value = "job", proxyMode = ScopedProxyMode.TARGET_CLASS)
    ...
    //fullstack
//    @Bean
//    public Job job4(JobRepository repository) {
//        return new JobBuilder("job4", repository)
////                .incrementer(new RunIdIncrementer())
//                .start(wontedFullStackStep(repository))
//                .build();
//    }
